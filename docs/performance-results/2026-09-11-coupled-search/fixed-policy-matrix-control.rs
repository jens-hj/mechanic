...
#[test]
fn captured_fixed_policy_matrix_control() {
    let mut passed = 0;
    for substeps in [1, 2, 4, 8] {
        println!("fixed_endpoint_policy substeps={substeps} tolerance=1e-8");
        let result = std::panic::catch_unwind(|| cold_drop(
            TerrainIntegration::SlowContactsAtEndpoint,
            JointTickSettings { substeps, maximum_substeps: substeps, ..Default::default() },
        ));
        println!("fixed_endpoint_policy substeps={substeps} complete={}", result.is_ok());
        passed += usize::from(result.is_ok());
    }
    assert_eq!(passed, 4, "fixed quality policies have not all passed");
}
