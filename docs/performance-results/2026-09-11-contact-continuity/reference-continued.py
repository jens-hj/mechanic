...
x=np.load('/private/tmp/mechanic-contact-reference.npz')['impulses']
for iteration in range(5000):
    F,A=evaluate(x); norm=np.linalg.norm(F)
    if iteration%250==0 or max(abs(F))<1e-10: print('iter',iteration,'res',max(abs(F)), 'norm',norm,flush=True)
