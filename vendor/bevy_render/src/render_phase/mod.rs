    ...
    BPI: BinnedPhaseItem,
{
    ...
    /// Batch keys in the exact traversal order used by `render_filtered`.
    /// Diagnostic consumers may partition contiguous draws without reordering them.
    pub fn draw_batch_keys(&self) -> Vec<&BPI::BatchSetKey> {
        let mut keys = Vec::new();
        match &self.batch_sets {
            BinnedRenderPhaseBatchSets::DynamicUniforms(sets) => {
                for ((key, _), batches) in self.batchable_meshes.keys().zip(sets) {
                    keys.extend(core::iter::repeat_n(key, batches.len()));
                }
            }
            BinnedRenderPhaseBatchSets::Direct(sets) => {
                keys.extend(
            ...
            BinnedRenderPhaseBatchSets::MultidrawIndirect(sets) => {
                keys.extend(
                    self.multidrawable_meshes
                        .keys()
                        .chain(self.batchable_meshes.keys().map(|(key, _)| key))
                        .take(sets.len()),
                );
            }
        }
        for ((key, _), entities) in &self.unbatchable_meshes {
            if !matches!(
                entities.buffer_indices,
                UnbatchableBinnedEntityIndexSet::NoEntities
            ) {
                keys.extend(core::iter::repeat_n(key, entities.entities.len()));
            }
        }
        for ((key, _), entities) in &self.non_mesh_items {
            keys.extend(core::iter::repeat_n(key, entities.entities.len()));
        }
        keys
    }
    ...
    pub fn render<'w>(
        ...
        self.render_filtered(render_pass, world, view, &mut |_| true)
    }

    /// Renders selected prepared draw items in their original order.
    /// The predicate visits each item once, including batched/indirect items.
    /// Returning false skips encoding that draw without changing batching.
    pub fn render_filtered<'w>(
        &self,
        render_pass: &mut TrackedRenderPass<'w>,
        world: &'w World,
        view: Entity,
        filter: &mut impl FnMut(&BPI) -> bool,
    ) -> Result<(), DrawError> {
        {
        ...
        self.render_batchable_meshes(render_pass, world, view, filter)?;
        self.render_unbatchable_meshes(render_pass, world, view, filter)?;
        self.render_non_meshes(render_pass, world, view, filter)?;
