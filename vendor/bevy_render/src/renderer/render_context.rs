...
/// Owned timestamp allocation for one tracked render pass.
pub struct RenderPassTimestampAllocation {
    /// Timestamp query set enabled on the render device.
    pub query_set: wgpu::QuerySet,
    /// Query written at the beginning of this actual render pass.
    pub beginning: u32,
    /// Query written at the end of this actual render pass.
    pub end: u32,
}

/// Optional diagnostic hook for passes begun through [`RenderContext::begin_tracked_render_pass`].
/// The callback receives the current view and pass label. It must return unique,
/// valid query indices, or `None` to skip measurement. Existing descriptor timestamp
/// writes are never replaced. Raw encoder passes are not intercepted.
#[derive(Resource, Clone)]
pub struct RenderPassTimestampHook(
    pub  Arc<
        dyn Fn(Option<Entity>, Option<&str>) -> Option<RenderPassTimestampAllocation> + Send + Sync,
    >,
);
...
pub struct RenderContext<'w, 's> {
    ...
    timestamp_hook: Option<Res<'w, RenderPassTimestampHook>>,
    current_view: Option<Res<'w, CurrentView>>,
}
...
impl<'w, 's> RenderContext<'w, 's> {
    ...
    pub fn begin_tracked_render_pass<'a>(
        ...
        let allocation = if descriptor.timestamp_writes.is_none() {
            self.timestamp_hook.as_ref().and_then(|hook| {
                (hook.0)(
                    self.current_view.as_ref().map(|view| view.0),
                    descriptor.label,
                )
            })
        } else {
        ...
        let descriptor = RenderPassDescriptor {
            timestamp_writes: descriptor.timestamp_writes.or_else(|| {
                allocation
                    .as_ref()
                    .map(|allocation| wgpu::RenderPassTimestampWrites {
                        query_set: &allocation.query_set,
                        beginning_of_pass_write_index: Some(allocation.beginning),
                        end_of_pass_write_index: Some(allocation.end),
                    })
            }),
            ..descriptor
        };
